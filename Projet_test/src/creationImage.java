...
@Override public void start(final Stage stage) throws Exception {
    ...
    layout1.getChildren().addAll(decor1,craft,Dossier,inventaire,texte,all,toolBar,buttonDel,buttonAdd,Matrice); //le Padre!
    ...
    buttonAdd.addEventHandler(MouseEvent.MOUSE_PRESSED,
            new EventHandler<MouseEvent>() {
              ...
              public void handle(MouseEvent e) {
            	  ...
            	  if(caMarche == true) {
            	  layout1.getChildren().clear();
                  layout1.getChildren().addAll(decor1,craft,Dossier,inventaire,texte,all,toolBar,buttonDel,buttonAdd,Matrice);//Remet fond craft
            	  caMarche = false;
            	  clef = clef/10*10;
            	  for(int i=0;i<10;i++) {
    ...
    all.setOnMouseReleased(evt -> {
        ...
        while(idico.hasNext() && caMarche == false && test ==false) {
        	...
        	if(compteur==craft_1.liste.size() && compteur==liste.size()) {
        		...
        		layout1.getChildren().clear();
        		layout1.getChildren().addAll(decor1,craftOk,Dossier,inventaire,texte,all,toolBar,buttonDel,buttonAdd,Matrice);//remplace fond craft
